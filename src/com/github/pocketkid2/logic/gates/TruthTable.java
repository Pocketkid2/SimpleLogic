...
public class TruthTable {
	...
	/**
	 * Gets the output column that matches the output character
	 *
	 * @param ch
	 *            The character to match output column with
	 * @return
	 */
	public boolean[] getOutput(char ch) {
		...
		boolean[] values = new boolean[table.length];

		// Loop through all rows and grab the right value
	...
	/**
	 * Inverts all bits on the output side of the truth table
	 */
	public void invert() {
		...
		for (int i = 0; i < table.length; i++) {
			...
			for (int j = 0; j < table[i].length; j++) {
				...
				// Grab the value, and store the logical NOT
				table[i][j] = !table[i][j];
			}
		}
	}
